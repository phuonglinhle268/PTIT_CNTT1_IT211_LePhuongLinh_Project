...
public class CourseService {
    ...
    public CourseResponse createCourse(CreateCourseRequest request) {
        ...
        if (request.getLecturerId() != null) {
            course.setLecturer(findLecturerOrThrow(request.getLecturerId()));
        }
    ...
    public void deleteCourse(Long id) {
        ...
        log.info("Admin đã xóa khóa học: {}", course.getCourseCode());
    }
    ...
    public CourseResponse enrollCourse(Long courseId, Long studentId) {
        Course course = courseRepository.findById(courseId)
                .orElseThrow(() -> new AppException("Không tìm thấy khóa học id: " + courseId, HttpStatus.NOT_FOUND));

        if (!course.getIsActive()) {
            throw new AppException("Khóa học này không còn hoạt động", HttpStatus.BAD_REQUEST);
        }
        ...
        if (student.getRole() != RoleEnum.STUDENT) {
            throw new AppException("Chỉ sinh viên mới được đăng ký khóa học", HttpStatus.FORBIDDEN);
        }

        // Kiểm tra đăng ký
        boolean alreadyEnrolled = courseRepository.isStudentEnrolled(courseId, studentId);
        if (alreadyEnrolled) {
        ...
        log.info("Sinh viên {} đăng kí khóa học {}", student.getUsername(), course.getCourseCode());
    ...
    public void unenrollCourse(Long courseId, Long studentId) {
        ...
        course.getEnrolledStudents().removeIf(u -> u.getId().equals(studentId));
        courseRepository.save(course);
        log.info("Sinh viên {} hủy khóa học với id {}", studentId, courseId);
    }
